function R_est = rotation_synch_MPLS(Z,A,opts)

opts.CEMP_parameters.max_iter = 5;
opts.CEMP_parameters.reweighting = 2.^((1:6)-1);
opts.CEMP_parameters.nsample = 5;
opts.MPLS_parameters.stop_threshold = 1e-1;
opts.MPLS_parameters.max_iter = 10;
...
% Generate indices and RijMat
Ind = [];
RijMat = zeros(3,3,1);
k = 1;
for i = 1:size(A,1)-1
    for j = i+1:size(A,1)
        if nnz(Z(3*i-2:3*i,3*j-2:3*j)) ~= 0
            Ind = [Ind; [i, j]];
            RijMat(:,:,k) = Z(3*i-2:3*i,3*j-2:3*j);
            k = k + 1;
        end
    end
end
...
%CEMP parameters
T=opts.CEMP_parameters.max_iter;
beta_cemp=opts.CEMP_parameters.reweighting;
nsample = opts.CEMP_parameters.nsample;
...
% MPLS paramters
stop_threshold=opts.MPLS_parameters.stop_threshold;
maxIters = opts.MPLS_parameters.max_iter;
beta_mpls = opts.MPLS_parameters.reweighting;
T_beta_mpls = length(beta_mpls);
if T_beta_mpls<maxIters
    beta_mpls = [beta_mpls,beta_mpls(end)*(ones(1,maxIters-T_beta_mpls))];
end

tau_mpls = opts.MPLS_parameters.thresholding;
T_tau_mpls = length(tau_mpls);
if T_tau_mpls<maxIters
    tau_mpls = [tau_mpls,tau_mpls(end)*(ones(1,maxIters-T_tau_mpls))];
end

alpha_mpls = opts.MPLS_parameters.cycle_info_ratio;
